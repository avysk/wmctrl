...
#define HELP                                                                   \
  ...
  "  -a <WIN>             Activate the window by switching to its desktop "    \
  "and\n"                                                                      \
  "                       raising it.\n"                                       \
  ...
  "below.\n"                                                                   \
  "  -r <WIN> -b <STARG>  Change the state of the window. Using this option "  \
  "it's\n"                                                                     \
  "                       possible for example to make the window "            \
  "maximized,\n"                                                               \
  ...
  "  -r <WIN> -T <STR>    Set both the name and the icon name of the "         \
  "window.\n"                                                                  \
  "  -k (on|off)          Activate or deactivate window manager's\n"           \
  "                       \"showing the desktop\" mode. Many window "          \
  "managers\n"                                                                 \
  ...
  "  -F                   Modifies the behavior of the window title "          \
  "matching\n"                                                                 \
  "                       algorithm. It will match only the full window "      \
  "title\n"                                                                    \
  ...
  "                       times. List of available workarounds is given "      \
  "below.\n"                                                                   \
  "\n"                                                                         \
  "Arguments:\n"                                                               \
  "  <WIN>                This argument specifies the window. By default "     \
  "it's\n"                                                                     \
  ...
  "                       of the title.\n"                                     \
  "\n"                                                                         \
  "                       The -i option may be used to interpret the "         \
  "argument\n"                                                                 \
  ...
  "argument\n"                                                                 \
  "                       as a string, which is matched against the "          \
  "window's\n"                                                                 \
  "                       class name (WM_CLASS property). Th first matching\n" \
  "                       window is used. The matching isn't case sensitive\n" \
  "                       and the string may appear in any position\n"         \
  "                       of the class name. So it's recommended to  always "  \
  "use\n"                                                                      \
  "                       the -F option in conjunction with the -x option.\n"  \
  "\n"                                                                         \
  "                       The special string \":SELECT:\" (without the "       \
  "quotes)\n"                                                                  \
  "                       may be used to instruct wmctrl to let you select "   \
  "the\n"                                                                      \
  "                       window by clicking on it.\n"                         \
  "\n"                                                                         \
  "                       The special string \":ACTIVE:\" (without the "       \
  "quotes)\n"                                                                  \
  "                       may be used to instruct wmctrl to use the "          \
  "currently\n"                                                                \
  ...
  "                       <G>,<X>,<Y>,<W>,<H>\n"                               \
  "\n"                                                                         \
  "                       <G>: Gravity specified as a number. The numbers "    \
  "are\n"                                                                      \
  "                          defined in the EWMH specification. The value "    \
  "of\n"                                                                       \
  "                          zero is particularly useful, it means \"use "     \
  "the\n"                                                                      \
  "                          default gravity of the window\".\n"               \
  "                       <X>,<Y>: Coordinates of new position of the "        \
  "window.\n"                                                                  \
  ...
  "Workarounds:\n"                                                             \
  "\n"                                                                         \
  "  DESKTOP_TITLES_INVALID_UTF8      Print non-ASCII desktop titles "         \
  "correctly\n"                                                                \
...
static int wm_info(Display *disp) { /*{{{*/
  ...
  if (!(sup_window =
    ...
    if (!(sup_window =
              (Window *)get_property(disp, DefaultRootWindow(disp), XA_CARDINAL,
  ...
  /* WM_PID */
  if (!(wm_pid = (unsigned long *)get_property(disp, *sup_window, XA_CARDINAL,
                                               "_NET_WM_PID", NULL))) {
    p_verbose("Cannot get pid of the window manager (_NET_WM_PID).\n");
  }

  /* _NET_SHOWING_DESKTOP */
  if (!(showing_desktop = (unsigned long *)get_property(
            disp, DefaultRootWindow(disp), XA_CARDINAL, "_NET_SHOWING_DESKTOP",
            NULL))) {
...
static int change_viewport(Display *disp) { /*{{{*/
  ...
  if (sscanf(options.param, "%lu,%lu", &x, &y) == 2) {
    return client_msg(disp, DefaultRootWindow(disp), "_NET_DESKTOP_VIEWPORT", x,
                      y, 0, 0, 0);
  } else {
    fputs(argerr, stderr);
    return EXIT_FAILURE;
...
static int change_geometry(Display *disp) { /*{{{*/
  ...
  if (sscanf(options.param, "%lu,%lu", &x, &y) == 2) {
    return client_msg(disp, DefaultRootWindow(disp), "_NET_DESKTOP_GEOMETRY", x,
                      y, 0, 0, 0);
  } else {
    fputs(argerr, stderr);
    return EXIT_FAILURE;
...
static int change_number_of_desktops(Display *disp) { /*{{{*/
  ...
  return client_msg(disp, DefaultRootWindow(disp), "_NET_NUMBER_OF_DESKTOPS", n,
                    0, 0, 0, 0);
} /*}}}*/
...
static int window_to_desktop(Display *disp, Window win, int desktop) { /*{{{*/
  ...
  if (desktop == -1) {
    if (!(cur_desktop = (unsigned long *)get_property(
              disp, root, XA_CARDINAL, "_NET_CURRENT_DESKTOP", NULL))) {
      if (!(cur_desktop = (unsigned long *)get_property(
                disp, root, XA_CARDINAL, "_WIN_WORKSPACE", NULL))) {
...
static int activate_window(Display *disp, Window win, /* {{{ */
  ...
  if ((desktop = (unsigned long *)get_property(
           disp, win, XA_CARDINAL, "_NET_WM_DESKTOP", NULL)) == NULL) {
    if ((desktop = (unsigned long *)get_property(
             disp, win, XA_CARDINAL, "_WIN_WORKSPACE", NULL)) == NULL) {
      p_verbose("Cannot find desktop ID of the window.\n");
    }
  }

  if (switch_desktop && desktop) {
    if (client_msg(disp, DefaultRootWindow(disp), "_NET_CURRENT_DESKTOP",
...
static int window_state(Display *disp, Window win, const char *arg) { /*{{{*/
  ...
  char *p1, *p2;
  const char *argerr =
      "The -b option expects a list of comma separated parameters: "
      "\"(remove|add|toggle),<PROP1>[,<PROP2>]\"\n";
    ...
    *p1 = '\0';
    ...
    if ((p2 = strchr(p1, ','))) {
      ...
      *p2 = '\0';
      ...
      tmp_prop2 =
          g_strdup_printf("_NET_WM_STATE_%s", tmp2 = g_ascii_strup(p2, -1));
      p_verbose("State 2: %s\n", tmp_prop2);
    ...
    tmp_prop1 =
        g_strdup_printf("_NET_WM_STATE_%s", tmp1 = g_ascii_strup(p1, -1));
    p_verbose("State 1: %s\n", tmp_prop1);
...
static int window_move_resize(Display *disp, Window win,
  ...
  const char *argerr = "The -e option expects a list of comma separated "
                       "integers: \"gravity,X,Y,width,height\"\n";
  ...
  if (grav < 0) {
    fputs("Value of gravity mustn't be negative. Use zero to use the default "
  ...
  if (wm_supports(disp, "_NET_MOVERESIZE_WINDOW")) {
    ...
    p_verbose("WM doesn't support _NET_MOVERESIZE_WINDOW. Gravity will be "
              "ignored.\n");
    if ((w < 1 || h < 1) && (x >= 0 && y >= 0)) {
...
static int list_desktops(Display *disp) { /*{{{*/
  ...
  if (!(num_desktops = (unsigned long *)get_property(
    ...
    if (!(num_desktops = (unsigned long *)get_property(
              disp, root, XA_CARDINAL, "_WIN_WORKSPACE_COUNT", NULL))) {
  ...
  if (!(desktop_geometry = (unsigned long *)get_property(
            disp, DefaultRootWindow(disp), XA_CARDINAL, "_NET_DESKTOP_GEOMETRY",
            &desktop_geometry_size))) {
    p_verbose(
        "Cannot get common size of all desktops (_NET_DESKTOP_GEOMETRY).\n");
  }

  /* desktop viewport */
  if (!(desktop_viewport = (unsigned long *)get_property(
            disp, DefaultRootWindow(disp), XA_CARDINAL, "_NET_DESKTOP_VIEWPORT",
  ...
  if (!(desktop_workarea = (unsigned long *)get_property(
    ...
    if (!(desktop_workarea = (unsigned long *)get_property(
              disp, DefaultRootWindow(disp), XA_CARDINAL, "_WIN_WORKAREA",
              &desktop_workarea_size))) {
  ...
  if (desktop_geometry && desktop_geometry_size > 0) {
    if (desktop_geometry_size == 2 * sizeof(*desktop_geometry)) {
      ...
      p_verbose("WM provides separate _NET_DESKTOP_GEOMETRY value for each "
                "desktop.\n");
      for (i = 0; i < *num_desktops; i++) {
  ...
  if (desktop_viewport && desktop_viewport_size > 0) {
    if (desktop_viewport_size == 2 * sizeof(*desktop_viewport)) {
      ...
      p_verbose("WM provides _NET_DESKTOP_VIEWPORT value only for the current "
                "desktop.\n");
      for (i = 0; i < *num_desktops; i++) {
        if (i == *cur_desktop) {
...
static int list_windows(Display *disp) { /*{{{*/
  ...
  for (i = 0; i < client_list_size / sizeof(Window); i++) {
    ...
    /* desktop ID */
    if ((desktop =
             (unsigned long *)get_property(disp, client_list[i], XA_CARDINAL,
                                           "_NET_WM_DESKTOP", NULL)) == NULL) {
      desktop = (unsigned long *)get_property(disp, client_list[i], XA_CARDINAL,
                                              "_WIN_WORKSPACE", NULL);
    }

    /* client machine */
    client_machine = get_property(disp, client_list[i], XA_STRING,
                                  "WM_CLIENT_MACHINE", NULL);

    /* pid */
    pid = (unsigned long *)get_property(disp, client_list[i], XA_CARDINAL,
                                        "_NET_WM_PID", NULL);

    /* geometry */
    XGetGeometry(disp, client_list[i], &junkroot, &junkx, &junky, &wwidth,
                 &wheight, &bw, &depth);
    XTranslateCoordinates(disp, client_list[i], junkroot, junkx, junky, &x, &y,
                          &junkroot);
